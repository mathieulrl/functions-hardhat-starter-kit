...
contract FunctionsConsumerDenigma is FunctionsClient, ConfirmedOwner {
   ...
   /**
     ...
     * @param rawAnswer The raw answer value toset
     */
    function setAnswer(string memory rawAnswer) external onlyOwner {
        answer = keccak256(bytes(rawAnswer));
    }
    
    /**
     * @notice Verify if a hash provided as a parameter is equal to the stored answer hash
     * @param hash The hash to verify
     * @return true if the hash is equal to the stored answer hash, false otherwise
     */
